...
4. **Social Compliance with Zero Intimate Violations:** We demonstrate through 20 randomized Monte Carlo physical trials that $\text{D}^2\text{RO}$ achieves **$0.0$ intimate personal space violations**, compared to $46.2 \pm 3.2$ violations in Static $A^*$, with negligible transit overhead.
5. **Sub-Millisecond Embedded Real-Time Feasibility:** We demonstrate that $D^*$ Lite incremental vertex repairs execute in **$0.04\text{ms}$ to $0.11\text{ms}$** across a $12\times$ scaling in obstacle density, consuming $<2.5\text{ KB/s}$ wireless mesh bandwidth and guaranteeing deterministic 60 FPS control on low-power microcontrollers (Raspberry Pi 5, Jetson Orin Nano).
6. **Multi-Domain Empirical Validation:** We validate the framework across three distinct architectural topologies (Retail Supermarket, Clinical Hospital, and Airport Terminal), providing 10 camera-ready 300 DPI figures and 4 open-access CSV datasets.
...
* **Section 5 (Experimental Results & Discussion):** Analyzes the 20-trial comparative benchmarks, component ablations, spatial proxemic heatmaps, spatiotemporal Turnout Alcove diagrams, and crowd density scalability curves.
